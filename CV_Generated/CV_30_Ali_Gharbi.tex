%-------------------------
% Resume in Latex
% Author : CV Generator
% License : MIT
%------------------------

\documentclass[letterpaper,11pt]{article}

\usepackage{latexsym}
\usepackage[empty]{fullpage}
\usepackage{titlesec}
\usepackage{marvosym}
\usepackage[usenames,dvipsnames]{color}
\usepackage{verbatim}
\usepackage{enumitem}
\usepackage[pdftex]{hyperref}
\usepackage{fancyhdr}

\pagestyle{fancy}
\fancyhf{}
\fancyfoot{}
\renewcommand{\headrulewidth}{0pt}
\renewcommand{\footrulewidth}{0pt}

% Adjust margins
\addtolength{\oddsidemargin}{-0.375in}
\addtolength{\evensidemargin}{-0.375in}
\addtolength{\textwidth}{1in}
\addtolength{\topmargin}{-.5in}
\addtolength{\textheight}{1.0in}

\urlstyle{same}

\raggedbottom
\raggedright
\setlength{\tabcolsep}{0in}

% Sections formatting
\titleformat{\section}{
  \vspace{-4pt}\scshape\raggedright\large
}{}{0em}{}[\color{black}\titlerule \vspace{-5pt}]

%-------------------------
% Custom commands
\newcommand{\resumeItem}[2]{
  \item\small{
    \textbf{#1}{: #2 \vspace{-2pt}}
  }
}

\newcommand{\resumeSubheading}[4]{
  \vspace{-1pt}\item
    \begin{tabular*}{0.97\textwidth}{l@{\extracolsep{\fill}}r}
      \textbf{#1} & #2 \\
      \textit{\small#3} & \textit{\small #4} \\
    \end{tabular*}\vspace{-5pt}
}

\newcommand{\resumeSubItem}[2]{\resumeItem{#1}{#2}\vspace{-4pt}}

\renewcommand{\labelitemii}{$\circ$}

\newcommand{\resumeSubHeadingListStart}{\begin{itemize}[leftmargin=*]}
\newcommand{\resumeSubHeadingListEnd}{\end{itemize}}
\newcommand{\resumeItemListStart}{\begin{itemize}}
\newcommand{\resumeItemListEnd}{\end{itemize}\vspace{-5pt}}

%-------------------------------------------
%%%%%%  CV STARTS HERE  %%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{document}

%----------HEADING-----------------
\begin{tabular*}{\textwidth}{l@{\extracolsep{\fill}}r}
  \textbf{\href{https://www.linkedin.com/}{\Large Ali Gharbi}} & Email : \href{mailto:ali.gharbi@gmail.com}{ali.gharbi@gmail.com}\\
  \href{https://www.linkedin.com/}{{linkedin.com/in/aligharbi}} & Mobile : +212 06 91 50 35 76 \\
\end{tabular*}

%-----------EDUCATION-----------------
\section{Education}
  \resumeSubHeadingListStart
    \resumeSubheading
      {FST Mohammedia}{Mohammedia}
      {Master in AI and Machine Learning;  CGPA: 7.91}{July. 2017 -- May. 2022}
  \resumeSubHeadingListEnd

%-----------EXPERIENCE-----------------
\section{Experience}
  \resumeSubHeadingListStart

    \resumeSubheading
      {Webhelp}{Casablanca}
      {Machine Learning Engineer}{Nov 2022 - Present}
      \resumeItemListStart
        \resumeItem{Task 1}
          {Built recommendation systems using collaborative filtering and deep learning}
        \resumeItem{Task 2}
          {Implemented NLP solutions for text analysis and sentiment detection}
        \resumeItem{Task 3}
          {Developed ML models for prediction and classification achieving 90\%+ accuracy}
      \resumeItemListEnd

    \resumeSubheading
      {BMCE Bank}{Casablanca}
      {Backend Developer}{May 2019 - Apr 2021}
      \resumeItemListStart
        \resumeItem{Task 1}
          {Architected and developed backend services using Python Django and PostgreSQL}
        \resumeItem{Task 2}
          {Built RESTful APIs following best practices with proper error handling and validation}
        \resumeItem{Task 3}
          {Developed message queue system using RabbitMQ for asynchronous task processing}
      \resumeItemListEnd

    \resumeSubheading
      {Accenture}{Casablanca}
      {Full Stack Developer}{Oct 2016 - Feb 2018}
      \resumeItemListStart
        \resumeItem{Task 1}
          {Created admin dashboard with analytics, reporting, and data visualization capabilities}
        \resumeItem{Task 2}
          {Built responsive web applications using React and Node.js with Redux state management}
        \resumeItem{Task 3}
          {Developed user authentication system with JWT and OAuth2 integration}
      \resumeItemListEnd

  \resumeSubHeadingListEnd

%-----------PROJECTS-----------------
\section{Projects}
  \resumeSubHeadingListStart
    \resumeSubItem{E-commerce Platform}
      {Developed a full-stack e-commerce platform with payment integration, product management, and order tracking using React and Node.js with PostgreSQL database.}
    \resumeSubItem{Inventory Management System}
      {Built a comprehensive inventory tracking system with barcode scanning, stock alerts, supplier management, and automated reporting.}
  \resumeSubHeadingListEnd

%--------PROGRAMMING SKILLS------------
\section{Skills}
 \resumeSubHeadingListStart
    \item{
     \textbf{Proficient}{: Django, NumPy, Ruby, Express.js, Kafka }
    }
    \item{
     \textbf{Comfortable}{: Microservices, Redis, SQL Server, Agile, DynamoDB}
    }
 \resumeSubHeadingListEnd

%-------------------------------------------
\end{document}